 ...
 *
 * There is a single layout today (struct nccl_ofi_gin_gdaki_qp / _cq below,
 * mirroring efa_cuda_qp / efa_cuda_cq). NCCL 2.31.0+ requests version 1;
 * version 0 is a vestigial table floor that predates the EFA-GDA backend and
 * is never sent by an EFA-GDA-capable NCCL. Both map to the current layout.
 *
 * INVARIANT: any change to the nccl_ofi_gin_gdaki_qp / _cq byte layout MUST
 * bump this value AND add a matching case to gdaki_gpu_qp/cq::build().
 * comp_mask capability bits are additive and never change the layout.
 */
