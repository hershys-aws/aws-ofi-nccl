...
#ifndef RMA_V14_H_
#define RMA_V14_H_
/* aws-ofi-nccl vendors net_v12.h (which defines ncclNetProperties_v12_t); NCCL upstream uses nccl_net.h. */
#include "net_v12.h"

/* Mirror of NCCL's ncclGinOptFlags (nccl_device/gin/gin_device_common.h), the values the GIN proxy passes
 * in the iput/iputSignal/iget optFlags argument below. The plugin cannot include that device-only header,
 * so the contract is vendored here next to the op-table it travels with. Keep in lock-step with NCCL; the
 * values are part of the dlopen ABI. */
enum {
  ncclGinOptFlagsDefault           = 0,
  ncclGinOptFlagsMaySkipCreditCheck = (1u << 0),
  ncclGinOptFlagsAggregateRequests  = (1u << 1),
};
...
typedef struct {
  ...
  // Put operations
  // optFlags carries ncclGinOptFlags (see nccl_device/gin/gin_device_common.h). The GIN proxy sources it
  // from the device-set GFD aggregate bit: ncclGinOptFlagsAggregateRequests => more ops follow on this
  // (context,rank), so the backend may defer the doorbell (e.g. set FI_MORE); the terminal op passes
  // ncclGinOptFlagsDefault to flush. Backends that do not coalesce may ignore it.
