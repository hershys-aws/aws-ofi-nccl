...
int nccl_ofi_gin_resources::flush_pending_doorbells()
{
	...
	/* Fast path: nothing deferred. This is also the steady state when NCCL
	   never sets the aggregate hint, so the common case costs one scan. */
	bool any = false;
	for (uint16_t r = 0; r < num_rails; r++) {
	...
	/* A rail has a write queued behind FI_MORE with no terminal op to ring its
	   doorbell (e.g. a pure-put burst). Ring each pending rail with a 0-byte
	   loopback write (no FI_MORE). Not bound to any comm. */
