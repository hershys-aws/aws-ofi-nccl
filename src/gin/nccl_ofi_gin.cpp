			       ...
			       uint32_t ack_seq_num, uint32_t count)
{
	...
	/* NOTE: if get_req_from_pool throws, ack_elem leaks. This matches
	   the existing pattern for metadata_send_req_t. */
	auto *req = gin_comm.resources.get_req_from_pool<nccl_net_ofi_gin_sendack_req_t>(
		gin_comm, ofi_ep, rail_id, ack_elem, rank_comm.address[rail_id], ack_fl);
